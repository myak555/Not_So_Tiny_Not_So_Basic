...
static byte TMODE_State = TMODE_DEGREES;
...
//
// Note on ESP32 Double is 8 bytes and Float (4 bytes) is not used
//
...
void loop()
{
  ...
  if( c == NL){    
    ...
    parser_Position = command;
    parse_Expression();
    //result = parse_Real( true);
    if(expression_Error){
      Serial.println("Ошибочка вышла!");
    }
    else{
      Serial.print("Получилось: ");    
      Serial.println(expression_Result);
    }
    counter = 0;
    *command = NULLCHAR;
    return;
