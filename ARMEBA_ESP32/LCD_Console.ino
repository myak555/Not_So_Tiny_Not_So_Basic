...
static void CONSOLE_PrintPROGMEM( const byte *msg){
  ...
  Serial.println( (char *)LCD_Message);
}
...
static void LCD_ScrollUp(){
  for( int i=0; i<LCD_SCREEN_ROWS-1; i++)
    strcpy( (char *)(LCD_Line_Pointers[i]), (char *)(LCD_Line_Pointers[i+1]));
}
...
//
static void LCD_PrintString( byte *msg, bool print_serial){
  LCD_ScrollUp();
  strncpy( (char *)(LCD_Line_Pointers[LCD_SCREEN_ROWS-1]), (char *)msg, LCD_TEXT_BUFFER_LINE_LENGTH);
  LCD_DrawScreen();
  if( print_serial) Serial.println( (char *)(LCD_Line_Pointers[LCD_SCREEN_ROWS-1]));
}
...
static void LCD_PrintPROGMEM( const byte *msg){
  ...
  LCD_DrawScreen();
  Serial.println( (char *)(LCD_Line_Pointers[LCD_SCREEN_ROWS-1]));
  LCD_Output_Keep = false;
}

//
// Returns number of printable positions in the string
// Currently Cyrillic Unicode markers 208 and 209 are supported
//
static size_t count_Unicode( byte *dest){
...
//
static size_t append_Message_String( byte *dest, const byte *msg, bool reset, bool unicode_count){
  size_t i = 0;
  size_t position_count = 0;
  if( !reset){
    i=Strlen( dest);
...
static void LCD_PrintProgLine( byte *ptr){
  snprintf( (char *)LCD_Message, LCD_TEXT_BUFFER_LINE_LENGTH, "%03u ", Program_Line_Number( ptr));
  append_Message_String( LCD_Message, Program_Line_Body( ptr), false, true);
  LCD_PrintString( LCD_Message, false);
  LCD_Output_Keep = false;
  snprintf( (char *)IO_Buffer, IO_BUFFER_LENGTH, "%04u ", Program_Line_Number( ptr));
...
static void LCD_PrintError(const byte *msg){
  ...
  *parser_Position = NL;
  if( parser_Position >= input_Top){
    ...
    snprintf( (char *)LCD_Message, LCD_TEXT_BUFFER_LINE_LENGTH, "%03u ", Program_Line_Number( current_Program_Line));
...
static void LCD_PrintQuoted(){
  ...
  if( report_ExpressionError( CONSOLE_ARGUMENT_MSG, false)) return;
...
//
static byte *LCD_ConvertDouble( double n, byte *buff){
  int i;
  if( n<0.0){
    *buff++ = '-';
    n=-n;
  ...
  *buff = NULLCHAR;
  ...
  if( n>9999999.0 || n<1.0){
    ...
    dtostrf( n, LCD_decimal_places+2, LCD_decimal_places, (char *)buff);
    i = Strlen( buff);
    snprintf( (char *)(buff+i), LCD_TEXT_BUFFER_LINE_LENGTH-i, "e%+03d", exponent);
    return buff;
  }
